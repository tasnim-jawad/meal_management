...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="card-title mb-3">Expenses (as per date)</h5>
        ...
        <div class="card-datatable table-responsive">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                <table class="datatables-users table border-top dataTable no-footer dtr-column text-center" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
                    ...
                    <tbody>
                    @foreach($result as $key => $single_date)
                        <tr>
                            <td>{{$key + 1}}</td>
                            @foreach ($single_date as $date => $amount)
                                <td>{{$date}}</td>
                                <td>{{$amount}}</td>
                                <td>
                                    <a href="{{ route('admin.daily_expense.expense_daily',$date) }}" class="btn btn-primary">Details</a>
                                </td>
                            @endforeach
                        </tr>
                    @endforeach
